...
namespace Compasso\Bpag\Model\Transactions;
    ...
    public function searchTransactions()
    {
        try {
            ...
            $select = $connection->select()
                ->from( ['order' => 'bpag_sales_order'], ['status', 'created_at', 'increment_id', 'store_id', 'entity_id'] )
                ->join( ['ps' => 'bpag_orders'], 'order.entity_id = ps.order_id')
            ...
            if ($this->_scopeConfig->getValue('payment/bpag/environment')) {
                $select = $select->where('ps.environment = ?', $this->_scopeConfig->getValue('payment/bpag/environment'));
            }
...
//            );
    ...
    {
        return $this->_order->load(\Compasso\Bpag\Helper\Data::getReferenceDecryptOrderID($payment->getReference()));
    }
    ...
    {
        return \Compasso\Bpag\Helper\Data::getStatusFromKey($status);
    }
    ...
    {
        return \Compasso\Bpag\Helper\Data::getPaymentStatusToString($status);
    }
    ...
    protected function getKeyFromOrderStatus($status)
    {
        ...
        return \Compasso\Bpag\Helper\Data::getKeyFromStatus($param);
    }
    ...
    {
        return \Compasso\Bpag\Helper\Data::getReferenceDecrypt($payment->getReference());
    }
    ...
    {
        $this->updateOrders('bpag_orders', $orderId, $transactionCode);
    }
    ...
    /**
     * Update column 'partially_refunded' the `bpag_orders` table
     *
     * @param int $orderId
     * @return void
     */
    protected function updatePartiallyRefundedPagSeguro($orderId)
    {
        $this->getConnection()
            ->query(sprintf(
                ...
                $this->getPrefixTableName('bpag_orders'),
                $orderId
            ));
    }
    ...
    protected function getPartiallyRefundedOrders()
    {
        ...
        $connection = $this->getConnection();
        $select = $connection->select()
            ->from( ['ps' => $this->getPrefixTableName('bpag_orders')], ['order_id'] )
    ...
    public function buildDetailsTransaction()
    {
        return array(
            ...
            'type'              => \Compasso\Bpag\Helper\Data::getTransactionTypeName($this->_detailsTransactionByCode->getType()),
            'status'            => \Compasso\Bpag\Helper\Data::getPaymentStatusToString($this->_detailsTransactionByCode->getStatus()),
            ...
            'cancelationSource' => \Compasso\Bpag\Helper\Data::getTitleCancellationSourceTransaction($this->_detailsTransactionByCode->getCancelationSource()),
    ...
    private function preparePaymentMethod()
    {
        ...
        if(!empty($this->_detailsTransactionByCode->getPaymentMethod()))
        {
            $paymentMethod = array(
                ...
                'titleType' => \Compasso\Bpag\Helper\Data::getTitleTypePaymentMethod($this->_detailsTransactionByCode->getPaymentMethod()->getType()),
                'titleCode' => \Compasso\Bpag\Helper\Data::getTitleCodePaymentMethod($this->_detailsTransactionByCode->getPaymentMethod()->getCode())
            );
        }
