...
namespace Compasso\Bpag\Controller\Direct;
...
class CreditCard extends \Magento\Framework\App\Action\Action
{
        ...
        try {
            ...
            $creditCard = new CreditCardMethod(
                ...
                $data = [
                    ...
                    'installment' => [
                        'parcela' => $this->getRequest()->getParam('installment_quantity'),
                        'valor'   => $this->getRequest()->getParam('installment_amount')
                    ],
    ...
    {
        echo "passou aqui compasso_bpag_aguardando_pagamento 1";
    ...
    private function whenSuccess($response)
    {
        ...
        return $this->result->setData([
            'success' => true,
            'payload' => [
                'redirect' => sprintf('%s%s', $this->baseUrl(), 'bpag/direct/success')
            ]
        ]);
    }
    ...
    private function makeSession()
    {
        $this->session()->setData([
            'bpag_payment' => [
                ...
                'order_id'      => $this->lastRealOrderId(),
                'nome'          => $this->getRequest()->getParam('holder_name')
            ]
        ]);
    }
